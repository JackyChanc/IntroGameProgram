...
void Entity::AIJump(Entity *target)
{
    ...
    case FOLLOWING:
        ...
        // Random jump when close to player and on ground
        if (mIsCollidingBottom && Vector2Distance(mPosition, target->getPosition()) < 150.0f) 
        {
            if (GetRandomValue(0, 100) < 50) // 50% chance per frame to jump
            {
...
int Entity::getCurrentFrameIndex() const { return mCurrentFrameIndex; }